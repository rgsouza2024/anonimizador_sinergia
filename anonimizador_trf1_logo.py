...
# Versão 1.5 - Ajustes finos no layout do cabeçalho
...
def carregar_analyzer_engine(termos_safe_location, termos_legal_header, lista_sobrenomes, termos_estado_civil, termos_organizacoes_conhecidas, termos_comuns_a_manter):
    ...
    if termos_comuns_a_manter:
        recognizer_common_terms = PatternRecognizer(supported_entity="TERMO_COMUM", name="CommonTermsRecognizer", deny_list=termos_comuns_a_manter, supported_language="pt", deny_list_score=1.0)
...
def gerar_arquivo_docx(texto):
    if not texto or texto.strip() == "": return None
    try:
        documento = Document()
...
with gr.Blocks(theme=gr.themes.Soft(), title="Anonimizador TRF1") as demo:
    ...
    # MUDANÇA: Adicionado vertical_align="center" na linha e ajustado tamanho da imagem e coluna
    with gr.Row(elem_id="header"):
        with gr.Column(scale=0, min_width=120): # Usar scale=0 para tamanho mínimo
            gr.Image(value=LOGO_FILE_PATH, interactive=False, show_download_button=False, show_label=False, width=100) # Tamanho reduzido
        with gr.Column(scale=4):
            gr.Markdown(
                """
                # Anonimizador SINERGIA
                **Ferramenta para anonimização de documentos.** 
    ...
    with gr.Tabs():
        with gr.TabItem("⌨️ Anonimizar Texto Colado"):
            with gr.Row():
                ...
                with gr.Column():
                    texto_anonimizado_area = gr.Textbox(lines=14, label="Texto Anonimizado", interactive=False, show_copy_button=True)
        ...
        with gr.TabItem("🗂️ Anonimizar Arquivo PDF"):
            ...
            with gr.Row():
                ...
                with gr.Column():
                    texto_anonimizado_pdf = gr.Textbox(lines=14, label="Texto Anonimizado", interactive=False, show_copy_button=True)
